...
export default function PartPanel({ part, accentClass, bpm }) {
  ...
  // bpm may arrive as a live number (from the tempo slider) or a string range.
  const numericBpm = typeof bpm === 'number' ? bpm : parseBpm(bpm)

  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      {/* Tips column */}
      <div className="border border-hairline bg-surface p-6 lg:col-span-1">
        <h3 className="font-display text-base font-bold uppercase tracking-wide text-ink mb-4 flex items-center gap-2">
          <span className="text-xl">{part.icon}</span> {part.name}
        </h3>
        <ul className="space-y-3">
          {part.tips.map((tip, i) => (
            <li key={i} className="flex gap-3 text-sm text-dim leading-relaxed">
              <span className={`mt-1.5 w-1.5 h-1.5 rounded-full bg-acid shrink-0`} />
              {tip}
            </li>
          ))}
        </ul>
      ...
      <div className="lg:col-span-2 space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          <div className="flex-1">
            <label className="font-mono text-[10px] text-dim uppercase tracking-[0.2em] block mb-1.5">
              Example pattern ({part.patterns.length} options)
            </label>
            <select
              value={selected}
              onChange={e => setSelected(Number(e.target.value))}
              className="w-full bg-surface-2 border border-hairline  px-4 py-2.5 text-sm text-ink focus:outline-none focus:border-acid cursor-pointer"
            >
  ...
  return (
    <div className="border border-hairline bg-surface p-5">
      <div className="flex items-center justify-between mb-4">
        <div>
          <p className="font-display text-base font-bold uppercase tracking-wide text-ink">{pattern.name}</p>
          <p className="text-xs text-dim">{typeLabel(pattern.type)} · {bpm} BPM</p>
        </div>
        {playable && (
...
function StepGrid({ steps, accentClass, activeStep }) {
  ...
  return (
    <div>
      <div className="flex gap-3">
        {beats.map((beat, bi) => (
          <div key={bi} className="flex-1">
            <div className="text-xs text-dim mb-1.5 font-mono">Beat {bi + 1}</div>
            <div className="flex gap-1.5">
              {beat.map((s, si) => {
                ...
                return (
                  <div
                    key={si}
                    className={`flex-1  h-16 flex items-end justify-center pb-1 transition-all duration-75 ${
                      s === 2
                        ? `bg-acid/50`
                        : s === 1
                        ? `bg-acid`
                  ...
                  >
                    <span className={`text-[10px] font-mono ${s ? 'text-base/60' : 'text-faint'}`}>
                      {si === 0 ? bi + 1 : si === 2 ? '&' : '·'}
                    </span>
                  </div>
                )
      ...
      {steps.some(s => s === 2) && (
        <p className="text-xs text-faint mt-3">Lighter shade = open hi-hat</p>
      )}
    </div>
  )
}
  ...
  return (
    <div>
      <div className="flex flex-wrap gap-1.5">
        {mapped.map((m, i) => {
          if (m.arrow) return <span key={i} className="px-1 py-2 text-acid font-mono text-sm self-center">→</span>
          const isActive = activeStep === m.step
          return (
            <span
              key={i}
              className={`px-3 py-2.5  text-sm font-mono min-w-[42px] text-center transition-all duration-75 ${
                m.rest
                  ? 'text-faint bg-surface-2'
                  : `bg-acid text-ink`
              } ${isActive ? 'ring-2 ring-ink scale-110' : ''}`}
            >
              {m.tok}
            </span>
          )
      ...
      <p className="text-xs text-faint mt-3">Each cell = one 8th-note step (– = rest, → = slide)</p>
    </div>
  )
}
...
function ChordGrid({ value, accentClass, activeStep }) {
  ...
  return (
    <div>
      <div className="flex flex-wrap gap-2">
        {chords.map((chord, i) => {
          ...
          return (
            <span
              key={i}
              className={`px-5 py-3  text-base font-semibold font-mono transition-all duration-75 ${
                isActive ? `bg-acid text-ink ring-2 ring-ink scale-105` : 'bg-surface-2 text-ink'
              }`}
            >
              {chord}
            </span>
      ...
      <p className="text-xs text-faint mt-3">Each chord plays for one beat · loops</p>
    </div>
  )
}
    ...
    <div className="flex flex-wrap gap-2 items-center">
      {sections.map((section, i) => (
        <div key={i} className="flex items-center gap-2">
          <span className="px-3 py-2  bg-surface-2 text-sm text-dim">{section.trim()}</span>
          {i < sections.length - 1 && (
            <svg className="w-3 h-3 text-faint shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
